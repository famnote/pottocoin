...
uint256 hashGenesisBlock("0x3f6cb435d39fc7189b0050b5692bfb3b0aeff1da8db6bbd99af8c1010115053f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pottocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pottocoin Signed Message:\n";
...
{
    // Pottocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Pottocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Pottocoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Pottocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pottocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pottocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Pottocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3f6cb435d39fc7189b0050b5692bfb3b0aeff1da8db6bbd99af8c1010115053f");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Pottocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PottocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PottocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PottocoinMiner(CWallet *pwallet)
{
    printf("PottocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pottocoin-miner");
    ...
    try { loop {
        ...
        printf("Running PottocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PottocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PottocoinMiner, pwallet));
}
